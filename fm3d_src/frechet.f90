...
!  print *,'finished ray section loop'
  ...
  if (locate_source) then
     ...
     if (ray%source%to_be_inverted) then
        ...
        dpos(1)= ray%section(1)%point(1,1)-raysec%point(1,2)
        dpos(2)= ray%section(1)%point(1,1)*(ray%section(1)%point(2,1)-raysec%point(2,2))
        dpos(3)= ray%section(1)%point(1,1)*cos(ray%section(1)%point(2,1))*(ray%section(1)%point(3,1)-raysec%point(3,2))

        dpos=dpos/sqrt(sum(dpos**2))

        vel=interpolate_velocity(ray%section(1)%point(1,1),ray%section(1)%point(2,1), &
             ray%section(1)%point(3,1),vgrid(ray%section(1)%reg%ivgrid,ray%section(1)%tf%vtype))
