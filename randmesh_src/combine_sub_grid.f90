...
integer,parameter:: mdim=ndep*nlat*nlon*2
...
velmean3d = reshape(velmean(1:mdim/2),(/ndep,nlat,nlon/))
stdvel3d = reshape(stdvel(1:mdim/2),(/ndep,nlat,nlon/))
print*,'mean variation for Vp and Vs:',maxval(abs(velmean(1:mdim/2))),maxval(abs(velmean(mdim/2+1:mdim)))
...
do m = 1,nvnp
idx = idx+1
velmean(k,l,m)=velref(idx)+velmean3d(k,l,m)
enddo
enddo
enddo
...
do l = 1,nvnt
do m = 1,nvnp
write(10,*),gop*rad2deg+(m-1)*gnsp*rad2deg,(got+(l-1)*gnst)*rad2deg,velmean(k,l,m)=velref(idx)+velmean3d(k,l,m)
enddo
enddo
enddo

velmean3d = reshape(velmean(mdim/2+1:mdim),(/ndep,nlat,nlon/))
write(10,*) nvnr,nvnt,nvnp
write(10,*) gnsr,gnst,gnsp
write(10,*) gor,got,gop
do k = 1,nvnr
...
enddo
enddo
stdvel3d = reshape(stdvel(mdim/2+1:mdim),(/ndep,nlat,nlon/))
do k = 1,nvnr
do l = 1,nvnt
do m = 1,nvnp
idx = idx+1
