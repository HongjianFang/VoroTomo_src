program generatemesh
  ...
  integer,parameter:: npts = 7200
  real,parameter:: radial = 6371.0
  real,parameter:: cmb = radial - 31
  real,parameter :: lat_s = 32.38,lat_n = 34.54,lon_w = 241.83,lon_e = 244.62
  real,parameter :: dlat = 0.01,dlon = 0.01, drad = 1
  integer,parameter:: nlat = 217,nlon = 280,nrad = 32 
...
!  allocate(theta_between(npts),phi_between(npts),rad_between(npts))
 ...
 ! rad_cmb = cmb 
  ...
  theta = theta*2*pi
  call slarnv(2,iseed,npts,phi)
  phi = acos(phi)
  call slarnv(1,iseed,npts,rad)
  rad = rad*(radial-cmb)+cmb
...
!!$omp end parallel
  ...
  close(34)
  inquire(iolength=reclen) C(1:2*nzall+1)
  write(filename,'("C_p",i0,".bin")'),inet
  open(34,file=filename,form='unformatted',access='direct',recl=reclen,status='replace')
  write(34,rec=1)C(1:2*nzall+1)
  close(34)
