...
import generaterandnum_rtt_loc
import generaterandnum_abs_loc
#import sys
import numpy as np
import calresidual
import generatedata
import generaterandnum_rtt
#import datetime
#
...
#
# inversion 
inv='../randmesh_src/relocation_absrtt'
...
subspaceproj='../randmesh_src/subspaceproj_absrtt'
...
class parallelrunsub (threading.Thread):
	def __init__(self,threadID,weight):
          threading.Thread.__init__(self)
	  self.threadID = threadID
	  self.weight = weight
	def run(self):
	  inet = self.threadID
	  weight = self.weight
          cmd1 = subspaceproj+ ' '+str(inet)+' '+str(weight)+' >> '+subpout
          cmd2 = backproj_ind+' '+str(inet)+' '+str(weight)+' >> '+ backprojout
...
def run_randmesh(runid,nomesh):
  ...
  if BGITER == 0:
    ...
    weightrtt = float(data[22].split()[2])
...
# uncomment this
  ...
  if BGITER==0 or (BGITER==1 and BINV==1 ):
    ...
    print('time for fm3d:',datetime.now())
    try:
...
##
  ...
  # and FMM
  #
  print('iterations:',NI,datetime.now())
  while ITER<=NI:
      if velinv == 1:
     # begin random mesh here
	 generatedata.randomchoosing()
	 generaterandnum_rtt.randomchoosing()
         ...
         for thidx in range(numfiles/nthread):
           ...
           for ii in range(1,nthread+1):
             t = parallelrunsub(thidx*nthread+ii,weightrtt)
             t.start()
             threads.append(t)
	 ...
	 print('time for vorotomo:',datetime.now())
	 ...
	 generaterandnum_rtt_loc.generate_rtt_loc()
	 generaterandnum_abs_loc.generate_abs_loc()
         os.system(inv + ' '+str(weightrtt))
...
# uncomment this for iterations
  ...
  copyfile('VoroTomo.in',runid+'/Vorotomo.in')
  end_time = datetime.now()
  print 'Duration: {}'.format(end_time-start_time)
  copyfile('output_sjfz.txt',runid+'/screenout.txt')
